...
MGraph::MGraph() : FGIM(), random_engine_(2024), enter_point_(0), reverse_(1 / log(1.0 * 20)), ef_construction_(200) {

}

MGraph::MGraph(unsigned int max_degree,
               unsigned int ef_construction,
               float sample_rate) : FGIM(max_degree, sample_rate), ef_construction_(ef_construction),
                                    random_engine_(2024), enter_point_(0),
                                    reverse_(1 / log(1.0 * max_degree)) {

}

void MGraph::CrossQuery(std::vector<IndexPtr> &indexes) {
    Timer timer;
    timer.start();
...
#pragma omp parallel for schedule(dynamic)
        for (size_t i = 0; i < graph_ref[0].size(); ++i) {
            ...
            for (size_t j = 0; j < neighbors.size() && j < max_degree_; ++j) {
                ...
                graph_[0][i + offset].candidates_.emplace_back(neighbor.id + offset, neighbor.distance, false);
            }
            std::make_heap(graph_[0][i + offset].candidates_.begin(), graph_[0][i + offset].candidates_.end());
        }
...
void MGraph::heuristic(Neighbors &candidates) {
...
void MGraph::ReconstructHGraph() {
    ...
    for (int u = 0; u < oracle_->size(); u++) {
        ...
        for (auto i = cur_max_level_; i > level; --i) {
            auto res = knn_search(oracle_.get(), visited_list_pool_.get(), graph_[i], (*oracle_)[u], 1, 1, cur_node_);
            cur_node_ = res[0].id;
        }
        ...
        for (auto i = std::min(level, cur_max_level_); i > 0; --i) {
            ...
            auto res = knn_search(
                    oracle_.get(), visited_list_pool_.get(), graph, (*oracle_)[u], ef_construction_,
                    ef_construction_,
                    cur_node_);
...
void MGraph::Combine(std::vector<IndexPtr> &indexes) {
...
Neighbors
MGraph::search(const float *query,
               unsigned int topk,
               unsigned int L) const {
    unsigned cur_node_ = enter_point_;
    for (int i = flatten_graph_.size() - 1; i > 0; --i) {
        auto res = graph::search(oracle_.get(), visited_list_pool_.get(), flatten_graph_[i], query, 1, 1, cur_node_);
        cur_node_ = res[0].id;
    }
    auto res = graph::search(oracle_.get(), visited_list_pool_.get(), flatten_graph_[0], query, topk, L, cur_node_);
    return res;
}
