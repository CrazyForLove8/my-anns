...
Kmeans::Kmeans(DatasetPtr &dataset,
               int k) : oracle_(dataset->getOracle()), k_(k), pointNumber_((int) dataset->getOracle()->size()) {
    centers_.reserve(k_);
    centers_.resize(k_);
...
std::vector<int> Kmeans::NearestCenter(int p,
                                       int ell) {
    std::vector<std::pair<int, float>> distance;
    for (int k = 0; k < k_; k++) {
        distance.emplace_back(k, (*oracle_)(p, centers_[k].id_));
    }
    std::sort(distance.begin(), distance.end(), [](const std::pair<int, float> &a,
