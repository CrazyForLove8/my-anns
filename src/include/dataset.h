...
namespace graph {
    ...
    loadGroundTruth(const std::string &filename,
                    unsigned int qsize,
                    unsigned int K = 100);
    ...
    public:
        ...
        void
        split(std::vector<std::shared_ptr<Dataset>> &datasets,
              unsigned int num);
        ...
        static
        std::shared_ptr<Dataset> aggregate(std::vector<std::shared_ptr<Dataset>> &datasets);
    };
